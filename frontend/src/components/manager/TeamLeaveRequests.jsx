...
import { getTeamLeaveRequests, reviewLeaveRequest } from '../../services/leaveService';
...
const TeamLeaveRequests = () => {
  ...
  const fetchLeaves = async () => {
    setLoading(true);
    try {
      const response = await getTeamLeaveRequests(filter);
  ...
  const handleReview = async (leaveId, status) => {
    ...
    try {
      const response = await reviewLeaveRequest(leaveId, status, managerNote);
